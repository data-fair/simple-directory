...
export default {
  ...
  common: {
    ...
    passwordShown: 'A sua senha está visível',
    passwordHidden: 'A sua senha está oculta',
    ...
    transfer: 'Transferir para outro site',
...
Pode ser 'anónimo', 'autenticado' ou 'administrador'.`,
  ...
  pages: {
    admin: {
      users: {
        ...
        editUserEmailTitle: 'Alterar o endereço de e-mail do usuário {name}',
        ...
        transferTitle: 'Transferir o usuário {name} para outro site',
        transferText: 'O usuário será vinculado ao site selecionado. Suas identidades SSO são redefinidas e ele mantém suas associações a organizações. Contas de superadministrador não podem ser transferidas.',
        transferTargetSite: 'Site de destino',
        transferMainSite: 'Site principal',
        drop2FAExplain: 'Atenção! Esta operação está reservada para desbloquear um utilizador que perdeu tanto o dispositivo como o código de recuperação para a autenticação de dois fatores. Antes de continuar, recomenda-se vivamente verificar de algum modo a identidade da pessoa que faz o pedido (por exemplo, contactando um colega da mesma organização).'
      },
      organizations: {
        limitOrganizationTitle: 'Mudar os limites organizacionais',
        members: 'membro(s)',
        nbMembers: 'Número máximo de membros (0 para sem limite)',
        fromCache: 'Última sincronização desta lista com o provedor de identidade: '
      },
      sites: {
    ...
    login: {
      ...
      recovery2FAInfo: 'Atenção! Guarde o código de recuperação abaixo em um local seguro. Sem ele, você não poderá recuperar sua conta se perder o dispositivo no qual acabou de configurar a autenticação de dois fatores.',
      ...
      authorizeApp: 'Autorizar aplicação',
      authorizeAppMsg: 'A aplicação {appName} pretende aceder à sua conta. Deseja autorizá-la?',
    ...
    organization: {
      ...
      departmentLabelHelp: 'Deixe vazio para mostrar "{defaultDepartmentLabel}". Preencha para utilizar outro vocabulário como "serviço", "agência", etc.',
      ...
      editNhi: 'Editar a conta de serviço',
      deleteNhi: 'Eliminar a conta de serviço',
      confirmDeleteNhiTitle: 'Eliminar a conta de serviço {name}',
      confirmDeleteNhiMsg: 'Deseja realmente eliminar esta conta de serviço? Os tokens em curso deixarão de funcionar assim que expirarem.',
      nhiIssuer: 'Emissor do token',
      nhiSubject: 'Sujeito',
      nhiAllowedIps: 'Endereços permitidos (opcional)',
      nhiAllowedIpsHint: 'Endereços ou sub-redes CIDR a partir dos quais a troca de token é aceita, por exemplo 203.0.113.5 ou 10.4.0.0/16. Deixe vazio para aceitar qualquer endereço.',
      nhiIpBinding: 'Vincular a sessão ao endereço a partir do qual foi obtida',
      nhiIpBindingHint: 'Um token vazado torna-se inutilizável de qualquer outro lugar. Ative apenas se esta conta de serviço chamar sempre a partir de um endereço estável: um endereço de saída variável (pool NAT, pilha dupla) quebraria suas sessões.',
      nhiJwks: 'JWKS (opcional, se o provedor não for acessível publicamente)',
      nhiClientId: 'Identificador de cliente (client_id)',
      nhiJwksInvalid: 'JSON inválido para o JWKS: {message}'
    },
    invitation: {
    ...
    me: {
      ...
      deleteMyselfAlert: 'Se apagar a sua conta, dispõe de um prazo de {plannedDeletionDelay} dias para cancelar a operação, após o qual todos os dados associados serão apagados e não poderão ser recuperados.',
      deleteMyselfCheck: 'marque esta caixa e clique em OK para confirmar a eliminação do utilizador {name} e de todos os seus dados.',
  ...
  errors: {
    ...
    adminChangeHostBlocked: 'Os superadministradores não podem transferir a sua conta para um site secundário. Inicie sessão a partir do site principal.',
    emailNotConfirmed: 'O seu endereço de email ainda não foi confirmado. Acabou de lhe ser enviada uma nova ligação de confirmação, abra-a para ativar a sua conta.',
