...
import { getMainSiteResources } from './main-site.ts'
 ...
 *
 * This mirrors reqSite(): on the main host the document is resolved through
 * getMainSiteResources, never read raw. Reading it raw here while the
 * /api/sites/_* endpoints read env is what made the immutable theme-css cache
 * key describe bytes it was not serving — the hash came from the document, the
 * CSS came from the environment, and the mismatch was cached for a year.
 */
export const getSiteExtraParams = async (siteUrl: string) => {
  const site = isMainSiteUrl(siteUrl) ? undefined : await getSiteByUrl(siteUrl)
  const mainResources = site ? undefined : await getMainSiteResources()
  return {
    THEME_CSS_HASH: site ? getThemeCssHash(site) : mainResources!.themeCssHash,
    PUBLIC_SITE_INFO_HASH: site ? getPublicSiteInfoHash(site) : mainResources!.publicInfoHash,
    ...
    SITE_TITLE: escapeHtml(site?.title || mainResources?.publicInfo.title || 'Simple Directory')
  }
}
