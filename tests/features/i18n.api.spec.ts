...
test.describe('i18n', () => {
  ...
  test('Error message with default lang if not specified', async () => {
    ...
    await assert.rejects(ax.get('/api/organizations/test_ihMQiGTaY/roles'), (res: any) => {
  ...
  test('Error message with default lang if unknown specified', async () => {
    const ax = await axiosAuth('dmeadus0@answers.com')
    await assert.rejects(ax.get('/api/organizations/test_ihMQiGTaY/roles', { headers: { 'Accept-Language': 'ff-FF' } }), (res: any) => {
      return res.data.includes('Permissions insuffisantes.')
    })
  })
  ...
  test('Error message with specified lang', async () => {
    const ax = await axiosAuth('dmeadus0@answers.com')
    await assert.rejects(ax.get('/api/organizations/test_ihMQiGTaY/roles', { headers: { 'Accept-Language': 'en-EN' } }), (res: any) => {
  ...
  test('Error message with another specified lang', async () => {
    const ax = await axiosAuth('dmeadus0@answers.com')
    await assert.rejects(ax.get('/api/organizations/test_ihMQiGTaY/roles', { headers: { 'Accept-Language': 'es-ES' } }), (res: any) => {
