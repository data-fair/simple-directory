...
export const startApiServer = async () => {
  ...
  // Override publicUrl to point directly to our in-process server (no nginx)
  ;(config as any).publicUrl = `http://localhost:${testPort}`
...
export const getDirectoryUrl = () => directoryUrl
export const getTestPort = () => testPort

export const axios = (opts: any = {}) => axiosBuilder({ baseURL: directoryUrl, ...opts, maxRedirects: 0 })
...
export const axiosAuth = async (opts: string | Omit<AxiosAuthOptions, 'password'> & { password?: string }) => {
  ...
  return _axiosAuth({ password, directoryUrl, ...opts, axiosOpts: { baseURL: directoryUrl, ...opts.axiosOpts } })
}
...
export const createUser = async (email: string, adminMode = false, password = 'TestPasswd01', _directoryUrl?: string) => {
  ...
  const ax = await axiosAuth({ email, adminMode, password, axiosOpts, directoryUrl: _directoryUrl ?? directoryUrl }) as AxiosAuthInstance
